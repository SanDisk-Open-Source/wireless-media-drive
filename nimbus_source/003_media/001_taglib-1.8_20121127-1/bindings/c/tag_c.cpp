...
BOOL taglib_save_thumbnail(TagLib_File *infile, char *target)
{
	//printf("taglib_save_thumbnail()\n");

	BOOL found = 0;
	File *f = reinterpret_cast<File *>(infile);

	//printf("f = %d\n", f);

	if (TagLib::MPEG::File* file = dynamic_cast<TagLib::MPEG::File*>(f))
	{
		if (file->ID3v2Tag())
		{
	...
	else if (TagLib::MP4::File* file = dynamic_cast<TagLib::MP4::File*>(f))
	{
		if (file->tag())
		{
	...
	else if (TagLib::FLAC::File* file = dynamic_cast<TagLib::FLAC::File*>(f))
	{
	...
	else if (TagLib::ASF::File* file = dynamic_cast<TagLib::ASF::File*>(f))
	{
		//printf("call ExtractASF()\n");

		found = ExtractASF(file, target);
	}
	else if (TagLib::APE::File* file = dynamic_cast<TagLib::APE::File*>(f))
	{
		if (file->APETag())
		{
			found = ExtractAPE(file->APETag(), target);
	...
	else if (TagLib::MPC::File* file = dynamic_cast<TagLib::MPC::File*>(f))
	{
		if (file->APETag())
		{
			found = ExtractAPE(file->APETag(), target);
	...
	else if (TagLib::WavPack::File* file = dynamic_cast<TagLib::WavPack::File*>(f))
	{
		if (file->APETag())
		{
			found = ExtractAPE(file->APETag(), target);
	  ...
	  //cout << "Can't found thumbnail data" << endl;
	}
