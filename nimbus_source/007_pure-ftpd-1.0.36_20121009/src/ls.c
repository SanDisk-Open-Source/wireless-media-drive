...
static int listfile(const PureFileInfo * const fi, const char *name)
{
    ...
    #if defined(QSI_PATCHED) && defined(QSI_DEBUG)
    printf("listfile> name=(%s)\n",name);
    #endif
    ...
    #ifdef QSI_PATCHED
    #ifdef QSI_DEBUG
    printf("listfile> filename=(%s)\n",name);
    #endif
    if(strcmp(name,FTP_HIDDEN_FOLDER_NAME)==0 && S_ISDIR(st.st_mode))
    {
        #ifdef QSI_DEBUG
        printf("<<found private>> pwd(%s),abs_path(%s)\n",wd,abs_path);
        #endif
        if(strcmp(wd,FTP_HIDDEN_PATH)==0 || strncmp(abs_path,FTP_HIDDEN_PATH,strlen(FTP_HIDDEN_PATH))==0)
        {
            if(strcmp(account,FTP_NOHIDDEN_ACCOUNT)!=0)
            {
                #ifdef QSI_DEBUG
                printf("No allow to access private folder!\n");
                #endif
                return 0;
            }
        }
                    ...
                    const char *name)
{
    ...
    if(strlen(name))
    {
        if(strcmp(account,FTP_ANONYMOUS_ACCOUNT)==0)
        {
            if(strcmp(wd,"/")==0)
            {
                sprintf(abs_path,FTP_ANONYMOUS_HOME "/%s",name);
            }
            else
            {
                sprintf(abs_path,FTP_ANONYMOUS_HOME "%s/%s",wd,name);
            }
        }
        else
        {
            sprintf(abs_path,"%s/%s",wd,name);
        }
    }

    #ifdef QSI_DEBUG
    // name: parameter of ls.
    printf("listdir> depth=%d,f=%d,name=(%s),pwd=(%s),root_directory=(%s),account(%s)\n",depth,f,name,wd,root_directory,account);
    printf("listdir> cmd=(%s),abs_path(%s)\n",cmd,abs_path);
    #endif
    ...
    while (s->name_offset != (size_t) -1) {
        ...
        if (FI_NAME(s)[0] != '.') {
            ...
            #ifdef QSI_PATCHED
            #ifdef QSI_DEBUG
            printf("listdir> filename=(%s)\n",FI_NAME(s));
            #endif
            if(strcmp(FI_NAME(s),FTP_HIDDEN_FOLDER_NAME)==0 && strncmp(abs_path,FTP_HIDDEN_PATH,strlen(FTP_HIDDEN_PATH))==0 )
            {
                #ifdef QSI_DEBUG
