...
/*
#ifdef QSI_PATCHED
    if(strstr(where,FTP_HIDDEN_FOLDER_NAME))
    {
        #ifdef QSI_DEBUG
        printf("docwd> found private\n");
        #endif
        if(!strcmp(account,FTP_ANONYMOUS_ACCOUNT))
        {
            printf("docwd> pwd(%d)where(%s)\n",wd,where);
            if(strcmp(wd,"/")==0)
            {
                sprintf(abs_path,FTP_ANONYMOUS_HOME "/%s",where);
            }
            else
            {
                sprintf(abs_path,FTP_ANONYMOUS_HOME "%s/%s",wd,where);
            }

            if(!strncmp(abs_path,FTP_HIDDEN_PATH,strlen(FTP_HIDDEN_PATH)))
            {
...
# endif            
    ...
    logfile(LOG_NOTICE, MSG_DELE_SUCCESS, root_directory,
        ...
        if (do_upload_script != 0 ) 
		{
			...
			{
		        snprintf(alloca_filename_real, sizeof_filename_real,"\001%s%s%s%s:DELE:", 
					root_directory,*name == '/' ? "" : wd,(*name != '/' && (!*wd || wd[strlen(wd) - 1] != '/')) ? "/" : "", name);
				l_ret = upload_pipe_push(account, alloca_filename_real);
				logfile(LOG_DEBUG, "QSI Delete file %s,%s,ret=%d",name,alloca_filename_real,l_ret);
        ...
        if (do_upload_script != 0 ) 
		{
			if ((alloca_filename_real = ALLOCA(sizeof_filename_real)) != NULL) 
			{
		        snprintf(alloca_filename_real, sizeof_filename_real,"\001%s%s%s%s:DELE:", 
					root_directory,*name == '/' ? "" : wd,(*name != '/' && (!*wd || wd[strlen(wd) - 1] != '/')) ? "/" : "", name);
				l_ret = upload_pipe_push(account, alloca_filename_real);
				logfile(LOG_DEBUG, "QSI RMD folder %s,%s,ret=%d",name,alloca_filename_real,l_ret);
				ALLOCA_FREE (alloca_filename_real);
			}
		}
#endif
...
void dornto(char *name)
{
    ...
    if (rename(renamefrom, name) < 0) {
        ...
        if (do_upload_script != 0 ) 
		{
			if ((alloca_filename_real = ALLOCA(sizeof_filename_real)) != NULL) 
			{
		        snprintf(alloca_filename_real, sizeof_filename_real,"\001%s%s%s%s:RENAME:%s%s%s%s", 
					root_directory,*renamefrom == '/' ? "" : wd,(*renamefrom != '/' && (!*wd || wd[strlen(wd) - 1] != '/')) ? "/" : "", renamefrom,
					root_directory,*name == '/' ? "" : wd,(*name != '/' && (!*wd || wd[strlen(wd) - 1] != '/')) ? "/" : "", name
					);
				l_ret = upload_pipe_push(account, alloca_filename_real);
				logfile(LOG_DEBUG, "QSI rename from %s,%s,ret=%d",name,alloca_filename_real,l_ret);
				ALLOCA_FREE (alloca_filename_real);
			}
		}
#endif
