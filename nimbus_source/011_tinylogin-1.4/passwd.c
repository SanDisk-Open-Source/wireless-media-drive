...
#ifdef QSI
	static int new_passwd_flag= 0;
	static char new_passwd_data[200] = {0};
#endif
...
extern int passwd_main(int argc, char **argv)
{
	...
	while ((flag = getopt(argc, argv, "a:dlu")) != EOF) {
		...
		case 'p':
			new_passwd_flag =1;
			strncpy (new_passwd_data,optarg,sizeof(new_passwd_data));
			break;
#endif			
		case 'd':
			dflg++;
...
#ifdef QSI
	if (!amroot && crypt_passwd[0] && new_passwd_flag == 0) {
#else
...
#ifdef QSI
	if (new_passwd_flag) {
		safe_strncpy(pass, new_passwd_data, sizeof(pass));
	}
	else
#endif
	{
