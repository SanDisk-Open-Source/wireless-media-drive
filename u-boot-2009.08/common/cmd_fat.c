...
char *sd_check_filename=NULL;
int sd_check_filename_result=0;
void nimbus_check_filename_callback(char *filename)
{
    if(sd_check_filename!=NULL)
    {
        if(strcmp(filename,sd_check_filename)==0)
        {
            printf("Found file:(%s)\n",filename);
            sd_check_filename_result=1;
            sd_check_filename=NULL;
        }
    }
}

int nimbus_check_filename(char *filename)
{
    block_dev_desc_t *dev_desc=NULL;
    int part=1,index;
    char *sd_filename=NULL;
    
    if( filename == NULL) return(0);

    dev_desc=get_dev("mmc",0);
    if( dev_desc == NULL) return(0);
    
    if (fat_register_device(dev_desc,part)!=0) return(0);
    sd_check_filename_result=0;
    sd_check_filename=filename;
    
    file_fat_ls ("/");

    if(sd_check_filename_result)
    {
        sd_check_filename_result=0;
        sd_check_filename=NULL;
        return(1);
