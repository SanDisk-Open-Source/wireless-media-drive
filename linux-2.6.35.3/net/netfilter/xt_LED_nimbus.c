...
extern void nimbus_idle_check_reset();
...
led_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
    ...
    if(strcmp("netfilter-ip",ledinfo->id)==0)
    {
        //netfilter-ip
//        printk("<INFO> package in - netfilter-ip\n");
        nimbus_idle_check_reset();
    }

    if(timer_pending(&ledinternal->timer))
    {
        //printk("xt_LED> timer pending\n");
        return XT_CONTINUE;
    }
