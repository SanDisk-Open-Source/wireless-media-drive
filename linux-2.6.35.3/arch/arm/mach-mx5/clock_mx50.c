...
#ifdef CONFIG_MACH_MX50_NIMBUS_USB_PATCH // Eric - for USB IF Test - EL_8,9
void enable_usb_charger(void)
{
    //Refer to i.MX508 Reference Manual P.585
    //printk(">Eric: Enable USB Charger Detector - %s() \n", __FUNCTION__);
    __raw_writel(0x10000, apll_base + MXC_ANADIG_MISC_CLR);
}
//EXPORT_SYMBOL_GPL(enable_usb_charger);

void disable_usb_charger(void)
{
    //Refer to i.MX508 Reference Manual P.585
    //printk(">Eric: Disable USB Charger Detector - %s() \n", __FUNCTION__);
    __raw_writel(0x10000, apll_base + MXC_ANADIG_MISC_SET);
}
//EXPORT_SYMBOL_GPL(disable_usb_charger);
#endif
