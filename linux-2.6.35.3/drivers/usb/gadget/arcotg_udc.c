...
#ifdef CONFIG_MACH_MX50_NIMBUS_USB_PATCH // Eric - control USB Charger Detector
...
extern void pmic_battery_turnoff_charging(void);
#endif
// in clock_mx50.c
extern void enable_usb_charger(void);
extern void disable_usb_charger(void);
...
#define CONFIG_FSL_USB_CHARGER
//#define CONFIG_TEST_FSL_USB_CHARGER
#define USB_CHARGER_DETECT_TIME 10000 
//#undef pr_debug
...
enum usb_battery_charger {
	...
	CHARGER_NOT_STANDARD /* for charger which dp and dm is not shorten */
 ...
 */
#if 0 // Eric - change the method, from USB.PHY to xxx
static void enable_usb_charger(void)
{
	u32 otg_phy_ctrl;

#if 1 // Eric - USB Charger
printk(">Eric: Enable USB Charger - %s() \n", __FUNCTION__);
#endif
...
#if 1 // Eric - USB Charger
printk(">Eric: Disable USB Charger - %s() \n", __FUNCTION__);
#endif
...
static void check_long_duration_plug_in_fn(unsigned long _udc)
{
	...
	/* if the usb cable is plugged out, do not need to check again */
	...
	if (((fsl_usb_battery_charger == CHARGER_SUSPEND_HOST) \
		...
		printk(KERN_INFO "Re-do charger detect\n");
		/* Pull down DP */
		...
		check_usb_charger();
       	} else if (((fsl_usb_battery_charger == CHARGER_SUSPEND_HOST) \
		|| (fsl_usb_battery_charger == CHARGER_NONE))\
	       	&& (temp & OTGSC_B_SESSION_VALID) \
		&& (detect_charger_counter > 1)) {
		fsl_usb_battery_charger = CHARGER_NOT_STANDARD;	
	}
...
static void usb_charger_handler(void)
{
	...
	if ((temp & PORTSCX_LINE_STATUS_BITS) == PORTSCX_LINE_STATUS_UNDEF) {
		pr_debug("%s: it is dedicated usb charger\n", __func__);
		fsl_usb_battery_charger = CHARGER_DCP;
	} else {
		pr_debug("%s: it is host charger\n", __func__);
		fsl_usb_battery_charger = CHARGER_CDP;
	}
}

#ifdef CONFIG_TEST_FSL_USB_CHARGER
static int usb_charger_thread(void *param)
{
	while(1) {
		msleep(10000); /* run every 10s */

		if (fsl_usb_battery_charger == CHARGER_DCP)
			printk(KERN_INFO "it is dedicated usb charger\n");
		else if (fsl_usb_battery_charger == CHARGER_DCP)
			printk(KERN_INFO "it is host charger\n");
		else if (fsl_usb_battery_charger == CHARGER_SUSPEND_HOST)
			printk(KERN_INFO "it is suspend host\n");
		else if (fsl_usb_battery_charger == CHARGER_ACTIVE_HOST)
			printk(KERN_INFO "it is ACTIVE host\n");
		else if (fsl_usb_battery_charger == CHARGER_NOT_STANDARD) {
			printk(KERN_INFO "it is not a standard charger, ");
			printk(KERN_INFO "may DP and DM is not shorten\n");
		} else
			printk(KERN_INFO "no usb cable is connected\n");
	}
	return 0;
}
#endif
...
// Eric - control USB Charger Detector
#if defined(CONFIG_MACH_MX50_NIMBUS_USB_PATCH) && defined(CONFIG_FSL_USB_CHARGER)
		check_usb_charger();
#else
		temp = fsl_readl(&dr_regs->usbcmd);
		temp |= USB_CMD_RUN_STOP;
		...
		struct usb_ctrlrequest *setup)
{
		...
		/* Status phase from udc */
	{
		...
		if (ptc) {
            ...
            #ifdef CONFIG_MACH_MX50_NIMBUS_USB_PATCH
            #ifdef CONFIG_MXC_MC13892_BATTERY
            pmic_battery_turnoff_charging();
            #endif
            #endif
...
#ifdef CONFIG_MACH_MX50_NIMBUS_USB_PATCH // Eric - control USB Charger Detector
 	/* connect to active host, do not re-assgin fsl_usb_batttery_charger
 	 * if it is a host charger 
 	 */
 	if (fsl_usb_battery_charger != CHARGER_CDP)
...
#if defined(CONFIG_MACH_MX50_NIMBUS_USB_PATCH) && defined(CONFIG_TEST_FSL_USB_CHARGER) // Eric - control USB Charger Detector
