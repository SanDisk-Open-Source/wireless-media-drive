...
extern void wlan_set_power(int on);
static unsigned int wlan_power_flag = POWER_OFF ;
#endif
...
extern int nimbus_get_sd1_cd(void);
extern int sd1_cd_manual;
#endif
...
void mxc_mmc_force_detect(int id)
{
    ...
    //printk("mxc_mmc_force_detect> id=%d,mxc_fix_chips[id]=%d\n",id , mxc_fix_chips[id]);
	...
	if (host->detect_irq && host->id!=0)
		return;
    #else
	if (host->detect_irq)
		return;
    #endif
    //printk("call schedule_work(&host->cd_wq),host->detect_irq=%d\n",host->detect_irq);
...
static void sdhci_set_power(struct sdhci_host *host, unsigned short power)
{
    ...
    //printk("sdhci_set_power: wlan_power_flag=%d,host->id=%d\n",wlan_power_flag,host->id);
    if(host->id == 1) // johnson, only active for correct bus
    {
    	if (power == (unsigned short)-1) {
    		if (wlan_power_flag & POWER_ON){
                //printk("sdhci_set_power: wlan_set_power(0)\n");
    		    wlan_set_power(0);
    			wlan_power_flag |= POWER_OFF ;
    			wlan_power_flag &= ~POWER_ON ;
    		}
    	} else {
    		if(wlan_power_flag & POWER_OFF){
                //printk("sdhci_set_power: wlan_set_power(1)\n");
    		    wlan_set_power(1);
    			wlan_power_flag |= POWER_ON ;
    			wlan_power_flag &= ~POWER_OFF ;
    		}
    ...
    else if ((host->power != power) && (host->regulator_mmc)) {
		if (power == (unsigned short)-1) {
...
static void esdhc_cd_callback(struct work_struct *work)
{
	...
	do {
        ...
        #ifdef CONFIG_MACH_MX50_NIMBUS
        // Johnson, SD1_CD : Normal in low, Card insert in high.
        if (host->id == 0) 
        {
#if 0        
            if(sd1_cd_manual)
            ...
            //printk("esdhc_cd_callback: Found SD1 cd_status: %d \n",cd_status);
        }
        else
        {
            if (cd_status)
				      ...
				      *pdev, int slot)
{
	...
	if (!host->detect_irq) {
        ...
        //printk("sdhci_probe_slot> johnson no_detect_irt (%d)\n",pdev->id);
		goto no_detect_irq;
	}
    #ifdef CONFIG_MACH_MX50_NIMBUS
    /* This is patched for manual issue cd detection.
     */
    if ((pdev->id >= 0) && (pdev->id < MXC_SDHCI_NUM))
			mxc_fix_chips[pdev->id] = chip;
    //printk("sdhci_probe_slot> johnson (%d)\n",pdev->id);
    //printk("sdhci_probe_slot> johnson mxc_fix_chips[pdev->id]=%d\n",chip);
	...
	do {
        ...
        if (host->id == 0) 
        {
            if (!ret)
    			...
    			set_irq_type(host->detect_irq, IRQF_TRIGGER_RISING);
            //printk("sdhci_probe_slot: Found SD1 changes: %d \n",ret);
        }
        else
        {
            if (ret)
	...
	if (host->detect_irq)
		...
		if ((pdev->id >= 0) && (pdev->id < MXC_SDHCI_NUM))
			mxc_fix_chips[pdev->id] = chip;
        //printk("sdhci_probe_slot> johnson mxc_fix_chips[%d]=0x%x\n",pdev->id,mxc_fix_chips[pdev->id]);
	}
      out4:
