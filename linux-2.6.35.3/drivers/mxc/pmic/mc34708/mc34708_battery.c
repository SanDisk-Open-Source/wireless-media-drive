...
static int ripley_charger_update_status(struct ripley_dev_info *di)
{
	...
	if (ret == 0) {
		...
		if (usbOnline != di->usb_charger_online) {
            ...
            else if(usb_type == USBHOST)
            {
                di->usb_charger_online = 1;
            }
            else
            {
                di->usb_charger_online = 2;
            }
            dev_info(di->usb_charger.dev, "usb cable status: %s , type=%d\n",
				 usbOnline ? "online" : "offline",usb_type);

            if(di->usb_charger_online )
            {
